...
x <- rnorm(n = n, mean = 10,sd = 1) # produces a random sample of n observations from a normally distributed variable with mean=10 and sd=1
...
sd(x)/sqrt(n) # estimate of Standard error of the mean. But what does this mean.
...
x <- rnorm(n = 500, mean = 10,sd = 1) # produces a random sample of 500 observations from a normally distributed variable with 
...
sample.means <- replicate(1000, Dist1(n=500)) # This replicates the function 1000 times. i.e. it repeatedly samples 500 individuals from a population and estimates the mean for 1000 samples.
