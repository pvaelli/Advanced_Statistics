...
# For people doing association mapping, or other genomic studies, this can be fruitfully utilized for going through the data. HOWEVER.. The apply family of functions dynamically grow the output vector/matrix, which while fast for small data sets, is terrible for really large data sets.
...
#Set the names for each row of your empty vector with the names of the levels of the factor. This is important for this type (iterating across a vector of factor levels).
...
ls <- levene.test(SCT, genotype:temp:line)
ls.lm <- lm(ls ~ genotype*temp*line)
...
model.extract.GxT <- by(dll.data, INDICES=list(dll.data$genotype, dll.data$temp), function (x) lm (SCT ~ line, data=x))
